...
#Fait : rien (la variable est dÃ©jÃ  de type factor)#
#(A faire : transformer en variable quanti pourrait donner une idÃ©e du nb moyen de prise alimentaire (borne infÃ©rieure). Pas sÃ»r que Ã§a soit Ã  faire cela dit.
#15 NA - s'en occuper#)
####

####
#A2_1_1 "[Prise alimentaire] [Heure] Ã€ quelle heure a eu lieu votre prise alimentaire  ?"
...
#"A2_1_2 "[Prise alimentaire] [Minute] Ã€ quelle heure a eu lieu votre prise alimentaire  ?"#
...
#A FAIRE :
#(
#(1)id_r = 479 a une rÃ©ponse aberrante ("12,30-19,00") - C'est un pannel#
#(2)il existe un certain nombre de rÃ©ponses problÃ©matiques Ã  recoder/vÃ©rifier : 05,30 /07,30/6,00/7,00/7,15/7,20/8,00.#
#2-1 - 5h30 : pain grillÃ© (ID_R 499)
#2-2 ID_R 437-439 "UN BOL DE CHOCOLAT DES TARTINES BEURRE UN VERRE DE JUS D ORANGE" & 07h30....PANNEL
#2-3 ID_R 567 MUESLI Ã  6h00....
#2-3 Que fait-on des 16h Ã  16H30 ?
#(3) J'ai laissÃ© cette variable en charactÃ¨re. En quoi faut-il la convertir ? Il est possible de la convertir en objet #time# Ã  l'aide du code as.POSIXct(enq.final$A2_re, format = "%H,%M"), mais "it turns out there's no pure "time" object, so every time must also have a date" donc Ã§a donne forcÃ©ment une date##
#(4) NA Ã  enquÃªter (231 ????!!!!!) = mÃªme nombre de NA que dans les variables d'origine, c'est Ã©trange - peut Ãªtre problÃ¨me d'importation ?#
#(5) Il est sans doute faux de considÃ©rer que lorsque qqn dit "19h", il voulait dire "19h,00")
#)
####


####
#A2_bis_1_1 [Heure] Ã€ quelle heure a eu lieu votre prise alimentaire principale (celle que vous considÃ©rez comme la plus importante) ?""
...
####
#"A2_bis_1_2" "[Prise alimentaire] [Minute] Ã€ quelle heure a eu lieu votre prise alimentaire principale (celle que vous considÃ©rez comme la plus importante) ?"
...
table(enq.final$A2_bis_re)
##
#NOTE : Je n'ai clairement pas pris le chemin le plus court pour faire ce que j'ai fait, mais nÃ©anmoins je l'ai fait.
#Note 2 : ce truc bizarre dans mon code ".              8,40" s'explique par la prÃ©sence de ce truc dans la bdd#

# A FAIRE -----OU PAS-----#
#1. Un certain nombre de valeurs problÃ©matiques : 13h, 16h, etc.
#2. Recoder "13,5"/22,9/9,5 ( sans doute 13,05/22,09/09,05) mais Ã  voir
#3. + mÃªme pb que pour la question prÃ©cÃ©dente (i.e quel type de variable))
####

####
#A3 "Combien de temps a-t-elle durÃ© ?"
...
#FAIT : rien (dÃ©jÃ  factor)#

#A faire : rien Ã  signaler (mais je me dis qu'on va galÃ©rer pour faire des moyennes vu qu'on a que des intervalles (sauf Ã  attribuer Ã  l'intervalle la moyenne de l'intervalle))
####

####
#A4 "Qui a prÃ©parÃ© ce que vous avez mangÃ© ?"
...
#FAIT : rien (dÃ©jÃ  en factor) #
#Remarque Gael : possible de regrouper les modalitÃ©s 2& 3#


#A FAIRE : rien Ã  signaler#
####

####
#A5 "Combien de temps vous a pris la prÃ©paration approximativement ?"
...
####
#A6 "Veuillez lister tout ce que vous avez mangÃ© Ã  cette occasion :"
...
#FAIT : rien (dÃ©jÃ  en caractÃ¨re)#
####

####
#A7 "ConsidÃ©rez-vous que c'Ã©tait un repas ?"
...
####
#A8 "Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ?
...
#Je la passe en caractÃ¨re avant de la repasser en factor parce que sinon j'ai un problÃ¨me lorsque j'essaie d'ajouter une modalitÃ© (les modalitÃ©s sont stockÃ©es sous forme de 1,2,...)"
enq.final$A8_re <- as.character(enq.final$A8)


#A8_other [Autre] Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ? 
...
#Je nettoie rapidement les modalitÃ©s (regroupement + forme)#
...
#Je les re-bascule dans la variable A8_re : les individus ayant choisi "Autre, prÃ©cisez" sont sinon codÃ©s comme NA#
enq.final$A8_re <- ifelse(is.na(enq.final$A8_other_re)==FALSE,"Autre, prÃ©cisez (voir A8_other_re)", enq.final$A8_re)
...
#A faire : ras#
##Remarque Mme Plessz : ordonner les modalitÃ©s (lÃ  autre s'est fouttu en 2Ã¨me position)#
#Question : que fait-on de danse et hopital ? CatÃ©gorie Autre semble pas mal#
####



####
#A9 "Est-ce que vous Ã©tiez Ã  table ? "
...
####
#A10  "Dans quelle piÃ¨ce vous-trouviez-vous ?"
...
enq.final$A10_other_re <-  ifelse(enq.final$A10_other_re=="bureau"|enq.final$A10_other_re=="BUREAU","Bureau",ifelse(enq.final$A10_other_re=="CHAMBRE FILS","Chambre Fils",ifelse(enq.final$A10_other_re=="dehors","Dehors",ifelse(enq.final$A10_other_re=="salle a manger"|enq.final$A10_other_re=="salle Ã  manger","Salle Ã  manger", ifelse(enq.final$A10_other_re=="terrasse","Terrasse",ifelse(enq.final$A10_other_re=="veranda"|enq.final$A10_other_re=="vÃ©randa","Veranda",enq.final$A10_other_re))))))
table(enq.final$A10_other_re)


#Je les re-bascule dans la variable#

enq.final$A10_re <- ifelse(is.na(enq.final$A10_other_re)==FALSE,"Autre (rÃ©ponse ouverte)", enq.final$A10_re)
enq.final$A10_re <- as.factor(enq.final$A10_re)
table(enq.final$A10_re)
table(enq.final$A10_other_re)

#FAIT : regroupement A10 et A10other#
#A FAIRE : rÃ©flexion sur le thÃ¨me: faut-il rebasculer des modalitÃ© de A10_other dans A10 ? on pourrait regrouper Terrasse + vÃ©randa + Dehors, faire basculer sale Ã  manger (presque autant que 1 piÃ¨ce),...Que faire de "bureau" ?#
## *PROBLEME* : il y a une modalitÃ© 7## ProblÃ¨me reglÃ© : j'avais oubliÃ© de rebasculer la variabel en caractÃ¨re#
####
...
#Je pense qu'il est mieux de laisser Ã§a sous la forme de variables dichotomiques (vu que QCM)#
...
enq.final$A11_other_re <-
  ifelse(enq.final$A11_other_re=="ecoutait la radio"|enq.final$A11_other_re=="ecouter la radio"|enq.final$A11_other_re=="radio","Oui, j'Ã©coutais la radio",ifelse(enq.final$A11_other_re=="donner a manger a ma fille","Oui, je donnais Ã  manger Ã  ma fille",ifelse(enq.final$A11_other_re=="Ã©couter de la musique"|enq.final$A11_other_re=="musique","Oui, j'Ã©coutais de la musique",ifelse(enq.final$A11_other_re=="regarder la tÃ©lÃ©vision", "Oui, je regardais la tÃ©lÃ©vision", enq.final$A11_other_re))))

table(enq.final$A11_other_re)

#CrÃ©ation d'une nouvelle variable
enq.final$A11_6_other_re <- ifelse(is.na(enq.final$A11_other_re)==FALSE,"Oui","Non sÃ©lectionnÃ©")
table(enq.final$A11_6_other_re)

#FAIT : crÃ©Ã© une dummy A11_6_other pour les rÃ©ponses autres ; nettoyÃ© les rÃ©ponses autres dans A11_6_other #
#A faire : 
# *probleme* modalitÃ© non pour a11_other_6_re 
#-Rebasculer le " Oui, je regardais la tÃ©lÃ©vision" en oui je regardais un Ã©cran
-#Regarder ce qu'il faut rebasculer en crÃ©ant une dummmy (tout ce qui a trait au son et Ã  la musique notamment)#
  ####
...
####
#A13_1 " "[Ami(s)] Avec qui Ã©tiez-vous ? "
...
enq.final$A13_5_other_re <- ifelse(enq.final$A13_5_other_re=="2 Petits enfants"|enq.final$A13_5_other_re=="mon petit fils de 3 ans","Petit enfant",ifelse(enq.final$A13_5_other_re=="aide menagere","Aide MÃ©nagÃ¨re",ifelse(enq.final$A13_5_other_re=="camarades de fac","Camarades de fac",enq.final$A13_5_other_re)))
table(enq.final$A13_5_other_re)

enq.final$A13_6_re <- ifelse(is.na(enq.final$A13_5_other_re)==FALSE, "Oui","Non sÃ©lectionnÃ©")

table(enq.final$A13_6_re)

#FAIT : crÃ©Ã© une dummy A13_6 pour les rÃ©ponses autres ; nettoyÃ© les rÃ©ponses autres dans A13_6_other #
# A FAIRE#
#Recodages
#*probleme* Ã§a nous a encore fait un 4 wtf* #PROBLEME RESOlU : fallait rebasculer en caractÃ¨re#
#"petit fils". TrÃ¨s Ã©trange d'avoir mis "famille"" et enfant. OÃ¹ mettre petit-fils (Ã  la fois "famille" et enfants (mais pas les siens !)) ? Si ce qui nous intÃ©resse, c'est l'Ã¢ge, alors recoder en enfant ; si c'est la nature du lien, alors recoder en famille.
#prÃ©sences forcÃ©es non prof ("collÃ¨gues") : colocataire/camarades de fac (non-amis)
#
####
...
####
"#A15 Les personne(s) qui Ã©taient avec vous regardaient-elles un Ã©cran ?"#
...
####
#A16 Parmi ces personnes qui regardaient un Ã©cran, certaines regardaient-elles le mÃªme Ã©cran que vous ? (Tv, Ordinateur, Tablette, TÃ©lÃ©phone/Smartphone, Console)"#
...
####
#A17_SQ001_SQ001 Parmi ces personnes qui regardaient un Ã©cran, certaines regardaient-elles le mÃªme Ã©cran que vous ? (Tv, Ordinateur, Tablette, TÃ©lÃ©phone/Smartphone, Console)"#
...
####
####
#A18 "Combien de temps a-t-elle durÃ© ? "#
...
####
#A19 "Qu'est-ce que vous avez mangÃ© ? "#
...
####
#A20 "Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ?"#
...
####
#A21 "Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ?"#
...
enq.final$A20_other_re <-ifelse(enq.final$A20_other_re=="opÃ©ra","OpÃ©ra",ifelse(enq.final$A20_other_re=="terrasse","Terrasse",ifelse(enq.final$A20_other_re=="thÃ©Ã¢tre","ThÃ©Ã¢tre",enq.final$A13_5_other_re)))
...
#cake	NA	thÃ©Ã¢tre
#sandwich	NA	opÃ©ra
# Ici, on est dans du repas sur le pouce/grignotage Ã  l'extÃ©rieur (non assis sans Ãªtre "en marchant"), et nous n'avions pas de catÃ©gorie#
####
...
enq.final$A21_other_re <- ifelse(enq.final$A21_other_re=="Cuisine"|enq.final$A21_other_re=="Je prÃ©parai le gaspacho du soir"|enq.final$A21_other_re=="je prepare la suite du repas petit pois diende"|enq.final$A21_other_re=="sauter les crepes","Oui, je faisais la cuisine", ifelse(enq.final$A21_other_re=="nettoyer"|enq.final$A21_other_re=="mÃ©nage"|enq.final$A21_other_re=="rangement, devoirs, menage", "Oui, je faisais du nettoyage et du mÃ©nage", ifelse(enq.final$A21_other_re=="musique","Oui, j'Ã©coutais de la musique",enq.final$A21_other_re)))

table(enq.final$A21_other_re)

enq.final$A21_6_re <- ifelse(is.na(enq.final$A21_other_re)==FALSE,"Oui","Non sÃ©lectionnÃ©")

#FAIT : CLEANAGE VAR OTHER (AVEC REGROUPEMENT) / + CREATION NVELLE DUMMY#

#A FAIRE : DISCUTER RECODAGE#
#MÃªme prb que prÃ©cÃ©demment pour musique + Questions des travaux mÃ©nagers (nettoyage, mÃ©nage, cuisine)#
#2 cents...anti-passivitÃ©....Ces gens remangent parce qu'ils font qqchose (activitÃ©) et qu'ils sont en contact avec de la nourriture...approche par le corps (activitÃ© + proximitÃ© avec bouffe comme prÃ©dicteur)...mais minoritaire (versus tÃ©lÃ©)#
####
...
####
#A22_SQ001_SQ001 [Heure] \tNous allons maintenant parler de votre derniÃ¨re prise alimentaire de la soirÃ©e. S'il s'agit de votre prise principale, parlez-nous de l'avant derniÃ¨re.\tÃ€ quelle heure a-t-elle eu lieu ?"#
...
####
#A24  "Qu'est-ce que vous avez mangÃ© ?"#
...
####
#A25 "Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ? "
...
####
#A25_other "Pouvez-vous me dire oÃ¹ vous Ã©tiez Ã  ce moment-lÃ  ? "
...
enq.final$A26_other_re <- ifelse(enq.final$A26_other_re=="Cuisine","Oui, je faisais la cuisine", ifelse(enq.final$A26_other_re=="Ã©couter un podcast radio","Oui, j'Ã©coutais un podcast radio",ifelse(enq.final$A26_other_re=="musique","Oui, j'Ã©coutais de la musique",enq.final$A26_other_re)))
table(enq.final$A26_other_re)

enq.final$A26_7_re <-ifelse(is.na(enq.final$A26_other_re)==FALSE,"Oui","Non sÃ©lectionnÃ©")
...
enq.final$A4_re <- factor(enq.final$A4_re,levels=c("Je lâ€™ai prÃ©parÃ©","Une autre personne du foyer lâ€™a prÃ©parÃ©","Plat Ã  emporter ou livraison Ã  domicile","Restaurant ou autre repas extÃ©rieur au domicile"))
table(enq.final$A4_re)
#GOOD#
...
enq.final$A8_re <- factor(enq.final$A8_re,levels=c("Chez vous","Au travail", "Dans les transports ou en marchant","Dans un restaurant/fast-food/brasserie etc.","Chez des amis","Chez de la famille","Autre, prÃ©cisez (voir A8_other_re)"))
table(enq.final$A8_re)
...
table(enq.final$A10_re)
enq.final$A10_re <- factor(enq.final$A10_re,levels=c("Cuisine","Salon / Salle de sÃ©jour", "Chambre","Mon logement a une seule piÃ¨ce","Autre (rÃ©ponse ouverte)"))
table(enq.final$A10_re)
#GOOD#
...
table(enq.final$A25_re)
enq.final$A25_re <- relevel(enq.final$A25_re,"Chez vous")
#PB : manquent modalitÃ©s vides p/r au dico des variables#
...
table(enq.final$A26_5_re)
##PB : QUID A26_4 ?? Vu que mÃªme vide, a crÃ©Ã© A26_6##
